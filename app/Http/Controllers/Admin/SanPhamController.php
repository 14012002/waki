...
class SanPhamController extends Controller
{
    ...
    {
        //
        $listSanPham = SanPham::with('danhMuc')->paginate(10);
        // dd($listSanPham);
        // Gọi đến view
    ...
    public function create()
    {
        ...
        $mau_sacs = MauSac::all();
        $dung_luongs = DungLuong::all();
        return view('admin.sanpham.add', compact('danh_mucs', 'mau_sacs', 'dung_luongs'));
    }
    ...
    public function store(SanPhamRequest $request)
    {
        if($request->isMethod('POST')){
            $params = $request->except('_token', 'mau_sac_ids', 'mau_sac_so_luongs', 'mau_sac_gias', 'dung_luong_ids', 'dung_luong_so_luongs', 'dung_luong_gias');
            ...
            // Xử lý màu sắc
            if ($request->has('mau_sac_ids')) {
                $mauSacData = [];
                foreach ($request->mau_sac_ids as $index => $mauSacId) {
                    $soLuong = $request->mau_sac_so_luongs[$index] ?? 0;
                    $gia = $request->mau_sac_gias[$index] ?? $request->gia;
                ...
                $sanPham->mauSacs()->attach($mauSacData);
            }
            
            // Xử lý dung lượng
            if ($request->has('dung_luong_ids')) {
                $dungLuongData = [];
                foreach ($request->dung_luong_ids as $index => $dungLuongId) {
                    $soLuong = $request->dung_luong_so_luongs[$index] ?? 0;
                    $gia = $request->dung_luong_gias[$index] ?? $request->gia;
                    
                    $dungLuongData[$dungLuongId] = [
                        'so_luong' => $soLuong,
                        'gia' => $gia
                    ];
                }
                $sanPham->dungLuongs()->attach($dungLuongData);
            }
